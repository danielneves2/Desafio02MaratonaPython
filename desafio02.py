...
def add_tracking_code(item,tracking_codes):
     tracking_codes.append(item)

def remove_tracking_code(code,tracking_codes):
    tracking_codes.remove(code)     

def get_tracking_code(indice,tracking_codes):
    return tracking_codes[indice]
    
def sum_two_lists(tracking_codes, company_two_tracking_codes):
    return (len(tracking_codes))+(len(company_two_tracking_codes))

def join_two_lists(tracking_codes,company_two_tracking_codes):
    return (tracking_codes)+(company_two_tracking_codes)

def get_the_biggest(last_sales):
    return max(last_sales)

def count_in_list ( item, last_sales):
    return last_sales.count(item)

def sum_total(last_sales):
    return sum(last_sales)
